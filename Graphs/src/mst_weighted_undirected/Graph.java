...
class Graph
{
	...
	private int nMarked;           // number of verts marked
	//-------------------------------------------------------------
	...
	public void mstw()           // minimum spanning tree
	   ...
	   while(nMarked < nVerts-1)   // while not all verts are marked
	      {                      // put currentVert in tree
	      vertexList[currentVert].isMarked = true;
	      nMarked++;
	      ...
	      for(int j=0; j<nVerts; j++)   // for each vertex,
	         ...
	         if(j==currentVert)         // skip if it's us
	            continue;
	         if(vertexList[j].isMarked) // skip if the dest vertex marked
	            continue;
	      ...
	      System.out.print(" ");
	      }  // end while(not all verts marked)
	
	   // mst is complete
	   for(int j=0; j<nVerts; j++)     // unmark vertices
	      vertexList[j].isMarked = false;
	   }  // end mstw
	//-------------------------------------------------------------
