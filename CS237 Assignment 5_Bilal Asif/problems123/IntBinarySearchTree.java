...
public class IntBinarySearchTree {
	...
	private boolean isBST(IntTreeNode root) {
		if (root == null)
			return true;
		if (isSubTreeLess(root.leftChild, root.key) && isSubTreeGreater(root.rightChild, root.key)
	...
	private boolean isSubTreeLess(IntTreeNode subRoot, int value) {
		if (subRoot == null)
			return true;
		if (subRoot.key < value && isSubTreeLess(subRoot.leftChild, value) && isSubTreeLess(subRoot.rightChild, value))
			return true;
		else
			return false;
	}
	...
	private boolean isSubTreeGreater(IntTreeNode subRoot, int value) {
		if (subRoot == null)
			return true;
		if (subRoot.key >= value && isSubTreeGreater(subRoot.leftChild, value)
	...
	public int nodeCount(IntTreeNode subTreeRoot) {
		...
		if (subTreeRoot == null) {//if nothing in tree
			return 0;
		} // end of if
		else {//else adding to each side
	...
	public int heightCount(IntTreeNode subTreeRoot) {
		if (subTreeRoot == null) {//if nothing in tree
			return 0;
		} // end of if
		else {//adding count to both sides
			int leftCount = heightCount(subTreeRoot.leftChild);
			int rightCount = heightCount(subTreeRoot.rightChild);

			// checking if returning larger count
